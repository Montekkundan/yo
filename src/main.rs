...
use yo::commands;
use yo::diagnostics::{self, DiagnosticEvent};
...
async fn run() -> anyhow::Result<()> {
    ...
    let args = Cli::parse();
    diagnostics::record(
        DiagnosticEvent::new("startup").outcome(true, started.elapsed().as_millis()),
    );
    let result: anyhow::Result<()> = async {
        match args.command {
            Some(Command::Setup { provider }) => commands::setup(provider).await?,
            ...
            Some(Command::Doctor { offline, json }) => {
                let report = yo::doctor::inspect(offline).await;
                yo::doctor::print(&report, json)?;
                if !report.healthy {
            ...
            Some(Command::Update { check }) => {
                if check {
                    let status = yo::update::check().await?;
                    if status.available {
                        println!("{} available", status.latest);
                    } else {
                        println!("{} is current", status.current);
                    }
                } else {
                    let result = yo::update::install_latest().await?;
                    if result.updated {
                        println!("Updated to {}", result.version);
                    } else {
                        println!("{} is current", result.version);
                    }
                }
            }
            Some(Command::Database(command)) => commands::database_command(command)?,
            Some(Command::Diagnostics(command)) => commands::diagnostics_command(command)?,
            Some(Command::Sandbox(command)) => commands::sandbox_command(command)?,
            Some(Command::Eval {
                filters,
                list,
                json,
